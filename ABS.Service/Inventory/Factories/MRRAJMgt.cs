...
{
    public class MRRAJMgt : iMRRAJMgt
    {
        ...
        public IEnumerable<vmGrr> GetMrrMasterList(vmCmnParameters objcmnParam, out int recordsTotal)
        {
            ...
            List<vmGrr> lstIssue = null;
            List<vmGrr> lstIssueWOPaging = null;
            recordsTotal = 0;
            try
            {
                lstIssueWOPaging = (from qcm in _ctxCmn.InvMrrMasters.Where(s => s.IsDeleted == false)                                    
                                    join org in _ctxCmn.CmnUsers on qcm.SupplierID equals org.UserID
                                    join usr in _ctxCmn.CmnUsers on qcm.UserID equals usr.UserID
                                    join ur in _ctxCmn.AccCurrencyInfoes on qcm.CurrencyID equals ur.Id
                                    select new vmGrr
                                    {
                                        MrrID = qcm.MrrID,
                                        MrrNo = qcm.MrrNo,
                                        SprNo = qcm.ReqNo,
                                        MrrDate  = qcm.MrrDate,
                                        ManualMRRNo = qcm.ManualMRRNo,
                                        CurrencyName = ur.CurrencyName,
                                        SupplierName = org.UserFullName,
                                        UserFullName = usr.UserFullName

                                    }).ToList();
                lstIssue = lstIssueWOPaging.OrderByDescending(x => x.MrrID).Skip(objcmnParam.pageNumber).Take(objcmnParam.pageSize).ToList();
