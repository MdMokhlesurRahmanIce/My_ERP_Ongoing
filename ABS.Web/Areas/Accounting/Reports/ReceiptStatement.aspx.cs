...
{
    public partial class ReceiptStatement : System.Web.UI.Page
    {
        ...
        protected void btnShow_Click(object sender, EventArgs e)
        {
            try
            {
                if (dpStartdate.Value != "" && dpEnddate.Value != "" && ddlCompanyName.SelectedValue != "")
                {
                    ...
                    ReportViewer1.Report = new ReceivedStatReport();
                    (ReportViewer1.Report as ReceivedStatReport).pvparam = UniqueCode.GetDateFormat_MM_dd_yyy(dpStartdate.Value);
                    (ReportViewer1.Report as ReceivedStatReport).pvparam1 = UniqueCode.GetDateFormat_MM_dd_yyy(dpEnddate.Value);

                    if (ddlAccountCode.Value == "" || ddlAccountCode.Value == String.Empty)
                    {
                        (ReportViewer1.Report as ReceivedStatReport).pvparam2 = null;
                    }
                    else
                    {
                        (ReportViewer1.Report as ReceivedStatReport).pvparam2 = ddlAccountCode.Value;
                    }

                    if (ddlCostCenter.SelectedValue == "" || ddlCostCenter.SelectedValue == String.Empty)
                    {
                        (ReportViewer1.Report as ReceivedStatReport).pvparam3 = null;
                    }
                    else
                    {
                        (ReportViewer1.Report as ReceivedStatReport).pvparam3 = ddlCostCenter.SelectedValue;
                    }

                    (ReportViewer1.Report as ReceivedStatReport).pvparam4 = ddlCompanyName.SelectedValue;
