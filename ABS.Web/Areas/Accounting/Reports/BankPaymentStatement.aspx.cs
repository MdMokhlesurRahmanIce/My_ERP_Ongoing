...
{
    public partial class BankPaymentStatement : System.Web.UI.Page
    {
        ...
        protected void btnShow_Click(object sender, EventArgs e)
        {
            try
            {
                if (dpStartdate.Value != "" && dpEnddate.Value != "" && ddlCompanyName.SelectedValue != "")
                {
                    ...
                    ReportViewer1.Report = new BankPaymentStatReport();
                    (ReportViewer1.Report as BankPaymentStatReport).pvparam =
                        UniqueCode.GetDateFormat_MM_dd_yyy(dpStartdate.Value);
                    (ReportViewer1.Report as BankPaymentStatReport).pvparam1 =
                        UniqueCode.GetDateFormat_MM_dd_yyy(dpEnddate.Value);

                    if (ddlAccountCode.Value == "" || ddlAccountCode.Value == String.Empty)
                    {
                        (ReportViewer1.Report as BankPaymentStatReport).pvparam2 = null;
                    }
                    else
                    {
                        (ReportViewer1.Report as BankPaymentStatReport).pvparam2 = ddlAccountCode.Value;
                    }

                    if (ddlCostCenter.SelectedValue == "" || ddlCostCenter.SelectedValue == String.Empty)
                    {
                        (ReportViewer1.Report as BankPaymentStatReport).pvparam3 = null;
                    }
                    else
                    {
                        (ReportViewer1.Report as BankPaymentStatReport).pvparam3 = ddlCostCenter.SelectedValue;
                    }

                    (ReportViewer1.Report as BankPaymentStatReport).pvparam4 = ddlCompanyName.SelectedValue;
