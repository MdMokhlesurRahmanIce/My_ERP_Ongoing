...
{
    public partial class CashPVReportByVoucherNo : System.Web.UI.Page
    {
        ...
        private void BindPV(int companyId)
        {
            try
            {
                using (var db = new ERP_Entities())
                {
                    //cbPVNO.Items.RemoveAt(0);
                    var source = db.AccVoucherMasters
                        .Where(r => r.VoucherTypeId == 1 && r.CompanyId == companyId)
                        .ToList();
        ...
        protected void btnShow_Click(object sender, EventArgs e)
        {
            try
            {
                if (cbPVNO.SelectedValue != "" && ddlCompanyName.SelectedValue != "")
                {
                    ReportViewer1.Report = new CashPaymentVoucherNew();
                    (ReportViewer1.Report as CashPaymentVoucherNew).pvparam = cbPVNO.SelectedValue;
                    (ReportViewer1.Report as CashPaymentVoucherNew).pvparam1 = ddlCompanyName.SelectedValue;
        ...
        protected void ddlCompanyName_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                var companyId = Convert.ToInt32(ddlCompanyName.SelectedValue);
