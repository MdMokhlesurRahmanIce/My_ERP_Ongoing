...
{
    public partial class JournalStatement : System.Web.UI.Page
    {
        ...
        protected void btnShow_Click(object sender, EventArgs e)
        {
            try
            {
                if (dpStartdate.Value != "" && dpEnddate.Value != "" && ddlCompanyName.SelectedValue != "")
                {
                    ...
                    ReportViewer1.Report = new JournalStatementReport();
                    (ReportViewer1.Report as JournalStatementReport).pvparam = UniqueCode.GetDateFormat_MM_dd_yyy(dpStartdate.Value);
                    (ReportViewer1.Report as JournalStatementReport).pvparam1 = UniqueCode.GetDateFormat_MM_dd_yyy(dpEnddate.Value);

                    if (ddlAccountCode.Value == "" || ddlAccountCode.Value == String.Empty)
                    {
                        (ReportViewer1.Report as JournalStatementReport).pvparam2 = null;
                    }
                    else
                    {
                        (ReportViewer1.Report as JournalStatementReport).pvparam2 = ddlAccountCode.Value;
                    }

                    if (ddlCostCenter.SelectedValue == "" || ddlCostCenter.SelectedValue == String.Empty)
                    {
                        (ReportViewer1.Report as JournalStatementReport).pvparam3 = null;
                    }
                    else
                    {
                        (ReportViewer1.Report as JournalStatementReport).pvparam3 = ddlCostCenter.SelectedValue;
                    }

                    (ReportViewer1.Report as JournalStatementReport).pvparam4 = ddlCompanyName.SelectedValue;
