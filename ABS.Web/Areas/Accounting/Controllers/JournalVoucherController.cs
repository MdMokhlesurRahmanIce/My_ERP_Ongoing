    ...
    public class JournalVoucherController : Controller
    {
        ...
        public JavaScriptResult Create(ViewModelVoucherMaster vmCpv)
        {
            try
            {
                ...
                var saveMessage = vmCpv.VoucherSave(userId,EmVoucherType.JournalVoucher,"JV");
                ...
                return JavaScript(string.Format("UYResult('{0}','{1}','{2}','{3}')",
                    "Data saved successfully.", "success", "redirect", Url.Content("~/Accounting/JournalVoucher/Create")));
            }
            catch (Exception ex)
            {
                return JavaScript(string.Format("UYResult('{0}','{1}')",
        ...
        public ActionResult Edit(ViewModelVoucherMaster vmCpv)
        {
            ...
            try
            {
                ...
                var editMessage = vmCpv.VoucherEdit(userId, EmVoucherType.JournalVoucher, "JV");
        ...
        public ViewResult BlankResultRow()
        {
            ...
            return View("_JournalVoucherDetail", result);
        }
        ...
        {
            List<AccVoucherMaster> soList =
                _db.AccVoucherMasters.Where(e => e.VoucherTypeId == 5).OrderByDescending(o => o.VoucherNo).ToList();
            ...
            if (!string.IsNullOrEmpty(param.sSearch))
            {
                ...
                filteredSo =
                    soList.Where(
                        c => isFirstNameSearchable && c.VoucherNo.ToLower().Contains(param.sSearch.ToLower())
            ...
            Func<AccVoucherMaster, string> orderingFunction =
                (c => sortColumnIndex == 1 && isStudentIdSortable
                    ? Convert.ToString(c.VoucherNo)
            ...
            var result = from c in displayedSo
                         select
                             new[] { c.VoucherNo, Convert.ToString(c.VoucherDate), Convert.ToString(c.IsActive) };
