...
app.controller('TopNavigationNotificationRPTCtrl', ['$scope', '$localStorage', 'sideNavService', function ($scope, $localStorage, sideNavService) {
    ...
    var hubNotif = $.connection.notificationHubsSales;

    $.connection.hub.start().done(function () {
        loadRecords_Notification(0);
    });

    hubNotif.client.updatedDataSales = function () {
        loadRecords_Notificationes(0);
    };

    hubNotif.server.broadcastDataSales = function (model) {
        loadRecords_Notification(0);
    };
