...
app.controller('SideMenuBarCtl', ['$scope', 'sideNavService', '$localStorage', '$rootScope',
    function ($scope, sideNavService, $localStorage, $rootScope) {
        var moduleID = 11; // It will come From Dynamic Top Navigation Bar Click Event
        //***************For Menu Permission ******************
        ...
        function loadRecords_Menues(dataModuleID) {
            ...
            var apiRoute = '/Sales/api/SalesLayout/GetSideMenu/';
            $scope.loaderWait = true;
            $scope.LoadMessage = 'Loading Please wait...!';

            var processMenues = sideNavService.GetSideMenu(apiRoute, $scope.sessionUserCompanyID, $scope.sessionLoggedUserID, ModuleID);
