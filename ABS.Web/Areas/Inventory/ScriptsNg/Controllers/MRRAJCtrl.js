...
app.controller('mRRAJCtrl', ['$scope', 'mRRService', 'qcService', 'RequisitionService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, mRRService, qcService, RequisitionService, conversion, $filter, $localStorage, uiGridConstants) {
        ...
        var baseUrl = '/Inventory/api/MRRAJ/';
        var isExisting = 0;
        var page = 0;
        var pageSize = 100;
        var isPaging = 0;
        ...
        function GetAllUsers() {
            var Route = '/Purchase/api/Quotation/GetAllUsers/';
            var UserLst = mRRService.getAllUsers(Route, page, pageSize, isPaging, UserTypeID, $scope.UserCommonEntity.loggedCompnyID);
        ...
        //**********---- Get All SPR Records ----*************** //
        var RequisitionTypeID = 8;
        $scope.getRequisition = function () {
            var apiRoute = '/Inventory/api/Issue/GetRequisitionNo/';
            var listRequisition = mRRService.getReqisitionlst(apiRoute, page, pageSize, isPaging, $scope.UserCommonEntity.loggedCompnyID, RequisitionTypeID);
        ...
        $scope.GetRequisitionItemList = function () {
            ...
            var RequisitionID = $scope.SprNo;
            if (RequisitionID != "") {
                ItemList = mRRService.getItemDetailsByRequisitionID(apiRoute, RequisitionID);
                ItemList.then(function (response) {
                    $scope.ListMrrDetails = response.data;
                    $scope.IsHiddenDetail = false;
                    //$scope.IsShowD = $scope.ListMrrDetails.length > 0 ? true : false;
                    //$scope.IsInvalid = $scope.ListMrrDetails.length > 0 ? false : true;
                    $scope.ManualSPRNo = response.data[0].ManualRequisitionNo;
        ...
        function loadWherehouse(isPaging) {
            ...
            listWherehouse.then(function (response) {
                ...
                $scope.listWarehouse.push({ OrganogramID: response.data.lstWherehouse[1].ListDpt[0].OrganogramID, OrganogramName: response.data.lstWherehouse[1].ListDpt[0].OrganogramName })
        ...
        $scope.loadMrrRecords = function (isPaging) {
            ...
            $scope.gridOptionsMrrMaster = {
                ...
                columnDefs: [
                    ...
                    { name: "MrrDate", displayName: $scope.grdTMrDate, title: $scope.grdTMrDate, cellFilter: 'date:"dd-MM-yyyy"', width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                     ...
                     { name: "SprNo", displayName: "SPR No", title: "SPR No", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    ...
                    { name: "CurrencyName", displayName: "Currency", title: "CurrencyName", visible: $scope.IsMrr, width: '10%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var MrrDetails = mRRService.GetList(apiRoute, cmnParam);
            ...
            //if($scope.IsMrr==true)
            //{
            //    cmnParam = "[" + JSON.stringify(objcmnParam) + "," + JSON.stringify(false) + "," + JSON.stringify(false) + "," + JSON.stringify(false) + "]";
            //}
            //else if ($scope.IsMrrAccountsDept == true) {

            //    cmnParam = "[" + JSON.stringify(objcmnParam) + "," + JSON.stringify(true) + "," + JSON.stringify(false) + "," + JSON.stringify(false) + "]";
            //}
            //else if ($scope.IsIssueReceive == true) {

            //    cmnParam = "[" + JSON.stringify(objcmnParam) + "," + JSON.stringify(false) + "," + JSON.stringify(false) + "," + JSON.stringify(false) + "]";
            //}
            //else if ($scope.IsReturnReceive == true) {
