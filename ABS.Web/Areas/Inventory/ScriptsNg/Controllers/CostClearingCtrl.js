...
app.controller('costClearingCtrl', ['$scope', 'gRRService', '$http', 'crudService', 'RequisitionService', 'mRRService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, gRRService, $http, crudService, RequisitionService, mRRService, conversion, $filter, $localStorage, uiGridConstants) {
        ...
        $scope.GetConsumerChargeType = function () {
            objcmnParam = {
                pageNumber: page,
                pageSize: pageSize,
                IsPaging: 0,
            ...
            var apiRoute = baseUrl + 'GetConsumerChargeType/';
            var listChargeType = $scope.GetList(apiRoute, cmnParam);
            listChargeType.then(function (response) {
                $scope.listChargeType = response.data.objPurchaseConsumerChargeType;
            },
                function (error) {
                    console.log("Error: " + error);
                });
        };
        $scope.GetConsumerChargeType();
      ...
      //  $scope.LoadInfoByPOID();
        ...
        $scope.loadChallanRecords = function (isPaging) {
            ...
            $scope.gridOptionsChallanMaster = {
                ...
                columnDefs: [
                    { name: "ClearingCostID", displayName: "ClearingCostID", visible: false, headerCellClass: $scope.highlightFilteredHeader },
                    ...
                    { name: "ClearingNo", displayName: "Clearing No", title: "Clearing No", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    ...
                    { name: "ClearingDate", displayName: "Clearing Date", cellFilter: 'date:"dd-MM-yyyy"', width: '20%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            var apiRoute = baseUrl + 'GetCostClearingMaster/';
        ...
        $scope.loadChallaMasterDetailsByGrrNo = function (dataModel) {
            ...
            $scope.HGRRNo = dataModel.ClearingNo;
            $scope.ClearingCostID = dataModel.ClearingCostID;
            $scope.ClearingDate = conversion.getDateToString(dataModel.ClearingDate);
            $scope.ClearingDescription = dataModel.ClearingDescription;
            ...
            var apiRoute = baseUrl + 'GetCostClearingDetailByCostClearingId/';
            var cmnParam = "[" + JSON.stringify(objcmnParam) + "]";
            var ListGrrDetails = gRRService.GetList(apiRoute, cmnParam);
            
            ListGrrDetails.then(function (response) {
                $scope.ClearingDetail = response.data.lstVmCostDetail;
            },
            function (error) {
                console.log("Error: " + error);
            });
        ...
        $scope.save = function () {
            ...
            var ClearingDate = conversion.getStringToDate($scope.ClearingDate);
                

            var itemMaster = {
                ClearingCostID: $scope.ClearingCostID,
                ClearingNo: $scope.ClearingNo,
                ClearingDate: ClearingDate,
                POID: $scope.PONo,
                RequisitionID: $scope.SprNo,
                CurrencyID: $scope.lstCurrencyList,
                ClearingDescription: $scope.ClearingDescription,
                DocUrl:$scope.DocUrl,
            ...
            var itemMasterDetail = $scope.ClearingDetail;
            var chkAmount = 1;
            angular.forEach($scope.ClearingDetail, function (item) {
                if (item.TaxValue <= 0) {
                    chkAmount = 0;
                }
            });
            ...
            if ($scope.ClearingDetail.length > 0) {
                ...
                if (chkAmount == 1) {
                    var apiRoute = baseUrl + 'SaveUpdateClearingCostMasterNdetails/';
